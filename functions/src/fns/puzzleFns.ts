...
export const uploadPuzzleSettings = functions.https.onCall(
  async (data, context): Promise<Record<string, string> | void> => {
    try {
      ...
      // changed new doc ID to publicKey for efficienct querying puzzles.
...
// return type set as an generic object bc a JSON is returned
// (Puzzle type is nested in that)
export const queryPuzzle = functions.https.onCall(
  async (data, context): Promise<Record<string, string> | void> => {
    try {
      const { publicKey } = data;

      // We should use the publicKey as the document id instead
      // of the (image) fileName.
      // This will let us retrieve the document directly by ID,
      // rather than query the
      ...
      if (puzzle.exists) {
        ...
        // add this puzzle to the user's received collection
