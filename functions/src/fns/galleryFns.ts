...
export const getDailyDates = functions.https.onCall(
  async (data, context): Promise<void | DailyDate[]> => {
    try {
      // throw error if user is not authenticated
      if (!context.auth) {
        throw new functions.https.HttpsError(
      ...
      const dailies = await db
        .collection("gallery")
        .where("dailyDate", ">=", timestamp)
        .orderBy("dailyDate")
        .limit(31)
        .get();
      if (!dailies.empty) {
        // this will include whatever data we want the user to submit w the puzzle
        ...
        const dailyPuzzles: DailyDate[] = dailies.docs.map((doc) => {
          const data = doc.data();
          return {
            ...data,
  ...
  async (data, context): Promise<void | DailyDate[]> => {
    try {
      ...
      if (!dailies.empty) {
        ...
        // @todo - decide on something to be done when no current daily,
        // e.g. get most recent
        return [];
      }
    } catch (error: unknown) {
