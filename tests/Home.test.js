...
describe('Home screen', () => {
  ...
  test('button for two grids is highlighted when pressed, and three grid button fades to background', () => {
    ...
    const twoGridButton = buttons.filter(button => button.props.children.includes('2'))
    const threeGridButton = buttons.filter(button => button.props.children.includes('3'))
    ...
    expect(surfaceForTwoButton.props.style.backgroundColor).toBe(theme.colors.surface)
    expect(surfaceForThreeButton.props.style.backgroundColor).toBe(theme.colors.background)
  })
})
